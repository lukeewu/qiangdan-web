...
class Users extends Controller
{
    ...
    public function index()
    {
       ...
       // $where1 = [['xy_convey.addtime','between',strtotime(date('Y-m-d')).','.time()]];
        $query->field('u.id,u.tel,u.username,u.lixibao_balance,u.id_status,u.ip,u.is_jia,u.addtime,u.invite_code,u.freeze_balance,u.status,u.balance,u.pipei_min, u.pipei_max,u1.username as parent_name,le.name as level_name,count(xy_convey.id) as dCount')
            ->leftJoin('xy_users u1','u.parent_id=u1.id')
            ->leftJoin('xy_level le','u.level=le.level')
            ->leftJoin('xy_convey xy_convey','u.id=xy_convey.uid')
			->group('u.id')
            ->where($where)
        //    ->where($where1)
            ->order('u.id desc')
            ...
            //           ];
            //     $day_d_count = Db::name('xy_convey')->where($where)->count('id');
            //     $vo->dCount = $day_d_count;
            //  }
...
// ->data($data)
    ...
    public function edit_users()
    {
        ...
        if(request()->isPost()){
            ...
            $jine = input('post.jine/s','');
            $danhao = input('post.danhao/s','');
            $shuliang = input('post.shuliang/s','');
            
            if($balance<0) $this->error('余额不能低于0');
            $res = model('Users')->edit_users($id,$tel,$user_name,$pwd,$parent_id,$balance,$pipei_min, $pipei_max,$freeze_balance,$token,$pwd2,$danhao,$jine,$shuliang);
        ...
        $this->level2 = Db::name('xy_goods_list')->field('goods_price')->group('goods_price')->order('goods_price asc')->select();
