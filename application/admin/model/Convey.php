...
class Convey extends Model
{
    ...
    private function rand_order($min,$max,$cid)
    {
        ...
        //卡单判断开始
        $uid = cookie('user_id');
        $uinfo1 = db('xy_users')->field('deal_status,status,balance,level,deal_count,danhao,jine,shuliang,deal_time,deal_reward_count dc')->find($uid);
        $lingdian = strtotime(date('Y-m-d').' 00:00:00');//零点
        $ershisidian = strtotime(date('Y-m-d').' 23:59:59');//24点
        //用户单号数组
        $danshu_array = explode(',',$uinfo1['danhao']);//分割
        //商品单价数组
        $jine_array = explode(',',$uinfo1['jine']);//分割
        //下单数量数组
        $shuliang_array = explode(',',$uinfo1['shuliang']);//分割
        $where = [
            ['uid','=',cookie('user_id')],
            ['addtime','between',strtotime(date('Y-m-d')).','.time()],
        ];
        $danshu_count = Db::name('xy_convey')->where($where)->where('status','in',[0,1,3,5])->count('id');
        if(!empty(($danshu_array))){
            for($i=0;$i<count($danshu_array);$i++){
                $val = $danshu_array[$i];
                    if($val == $danshu_count +1){//对比命中
                    if((count($jine_array) > $i) && (count($shuliang_array) > $i) ){
                                    $jineNum =$jine_array[$i];
                                    $shuliangNum =$shuliang_array[$i];
                            //查询该单价下的商品
                             $priceGoods = Db::name('xy_goods_list')
                              ->orderRaw('rand()')
                             ->where('goods_price','=',floatval($jineNum))
                             ->find();
                             if(!$priceGoods){
                              //    echo json_encode(['code'=>1,'info'=>lang('系统繁忙,请稍后重试！！！')]);die;
                             }
                            if ($priceGoods) {
                                //商品不为空的话，就改值
                                 $count =  $shuliangNum;
                                 $num =  $jineNum*$shuliangNum;
                                // echo json_encode(['code'=>1,'info'=>lang('有数据').$count.':'.$num]);die;
                                 return ['count'=>$shuliangNum,'id'=>$priceGoods['id'],'num'=>$num,'cid'=>$priceGoods['cid']];
                            }
