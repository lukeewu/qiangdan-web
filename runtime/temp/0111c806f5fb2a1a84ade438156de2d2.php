<?php /*a:2:{s:67:"/www/wwwroot/qiangdan/application/admin/view/deal/deposit_list.html";i:1616075732;s:54:"/www/wwwroot/qiangdan/application/admin/view/main.html";i:1615598992;}*/ ?>
<div class="layui-card layui-bg-gray"><style>        .layui-tab-card>.layui-tab-title .layui-this {
            background-color: #fff;
                  ...
                  if(isset($_GET['is_jia'])){
                    ...
                    ?> >假人</option></select></div></div><div class="layui-form-item layui-inline"><button class="layui-btn layui-btn-primary"><i class="layui-icon">&#xe615;</i> 搜 索</button><?php if(auth("do_deposit")): ?><a href="/admin/deal/daochu.html" class="layui-btn layui-btn-danger" ><i class="layui-icon">&#xe615;</i> 导 出</a><?php endif; ?></div></form></fieldset><fieldset><legend>数据统计</legend><font color="red">交易总额：<?php echo htmlentities((isset($depoist_sum) && ($depoist_sum !== '')?$depoist_sum:'0.00')); ?>元</font></fieldset><script>form.render()</script><table class="layui-table margin-top-15" lay-skin="line"><?php if(!(empty($list) || (($list instanceof \think\Collection || $list instanceof \think\Paginator ) && $list->isEmpty()))): ?><thead><tr><th class='list-table-check-td think-checkbox'><input data-auto-none data-check-target='.list-check-box' type='checkbox'></th><th class='text-left nowrap'>订单号</th><th class='text-left nowrap'>手机号</th><!--<th class='text-left nowrap'>姓名</th> --><th class='text-left nowrap'>真实姓名</th><th class='text-left nowrap'>上级用户</th><th class='text-left nowrap'>第几次提现</th><th class='text-left nowrap'>提现金额</th><th class='text-left nowrap'>手续费</th><th class='text-left nowrap'>实际到账</th><th class='text-left nowrap'>收款信息</th><th class='text-left nowrap'>发起时间</th><th class='text-left nowrap'>处理时间</th><!--<th class='text-left nowrap'>方式</th><th class='text-left nowrap'>二维码</th> --><th class='text-left nowrap'>订单状态</th><?php if(auth('do_deposit')): ?><th class='text-left nowrap'>操作</th><?php endif; ?></tr></thead><?php endif; ?><tbody><?php foreach($list as $key=>$vo): ?><tr><td class='list-table-check-td think-checkbox'><input class="list-check-box" value='<?php echo htmlentities($vo['id']); ?>' type='checkbox'></td><td class='text-left nowrap'><?php echo htmlentities($vo['id']); ?></td><td class='text-left nowrap'><?php echo htmlentities($vo['tel']); ?></td><!--<td class='text-left nowrap'><?php echo htmlentities($vo['username']); ?></td> --><td class='text-left nowrap'><?php echo htmlentities($vo['khname']); ?></td><td class='text-left nowrap'><?php echo htmlentities($vo['up_username']); ?></td><td class='text-left nowrap'><?php echo htmlentities($vo['nums']); ?></td><td class='text-left nowrap'>¥<?php echo htmlentities($vo['num']); ?></td><td class='text-left nowrap'><?php echo htmlentities($vo['shouxu']*100); ?>%</td><td class='text-left nowrap'>¥<?php echo htmlentities($vo['real_num']); ?></td><td class='text-left nowrap'><?php if ($vo['type'] != 2) { ?><div>银行:<?php echo htmlentities($vo['bankname']); ?></div><div>卡号:<?php echo htmlentities($vo['cardnum']); ?></div><div>支店名:<?php echo htmlentities($vo['back_name1']); ?></div><div>支店番号:<?php echo htmlentities($vo['back_name2']); ?></div><div>口座種類:<?php echo htmlentities($vo['back_name3']); ?></div><div>口座番号:<?php echo htmlentities($vo['back_name4']); ?></div><div>名義人:<?php echo htmlentities($vo['back_name5']); ?></div><?php } else { ?><div>支付宝:<?php echo htmlentities($vo['zhifunum']); ?></div><?php } ?></td><td class='text-left nowrap'><?php echo htmlentities(format_datetime($vo['addtime'])); ?></td><td class='text-left nowrap'><?php echo htmlentities(format_datetime($vo['endtime'])); ?></td><!--
            <td class='text-left nowrap'><?php if(($vo['type'] == 'wx')): ?><span class="layui-btn layui-btn-xs">微信</span><?php endif; if(($vo['type'] == 'zfb')): ?><span class="layui-btn layui-btn-xs layui-btn-warm">支付宝</span><?php endif; ?></td><td class='text-left nowrap'><?php if(($vo['type'] == 'wx')): ?><a data-dbclick data-title="查看图片" data-modal='<?php echo url("admin/users/picinfo"); ?>?pic=<?php echo htmlentities($vo['wx_ewm']); ?>'><img src="<?php echo htmlentities($vo['wx_ewm']); ?>" style="width:150px;height:100px;"></a><?php endif; if(($vo['type'] == 'zfb')): ?><a data-dbclick data-title="查看图片" data-modal='<?php echo url("admin/users/picinfo"); ?>?pic=<?php echo htmlentities($vo['zfb_ewm']); ?>'><img src="<?php echo htmlentities($vo['zfb_ewm']); ?>" style="width:150px;height:100px;"></a><?php endif; ?></td> --><td class='text-left nowrap'><?php switch($vo['status']): case "1": ?>待审核<?php break; case "2": ?>审核通过<?php break; case "3": ?>审核驳回<?=$vo['remark'] ? '(' . $vo['remark'] . ')' : '';break; case "4": ?>处理中<?php break; ?><?php endswitch; ?></td><td class='text-left nowrap'><?php if(($vo['status'] == 1) and auth("do_deposit")): ?><a class="layui-btn layui-btn-xs" data-csrf="<?php echo systoken('admin/deal/do_deposit'); ?>" data-action="<?php echo url('do_deposit'); ?>" data-value="id#<?php echo htmlentities($vo['id']); ?>;status#2">通过</a><a class="layui-btn layui-btn-xs layui-btn-warm J_bohui" data-csrf="<?php echo systoken('admin/deal/do_deposit'); ?>" data-url="<?php echo url('do_deposit'); ?>" data-id="<?php echo htmlentities($vo['id']); ?>" data-status="3" data-uid="<?php echo htmlentities($vo['uid']); ?>" data-num="<?php echo htmlentities($vo['num']); ?>" data-value="id#<?php echo htmlentities($vo['id']); ?>;status#3;uid#<?php echo htmlentities($vo['uid']); ?>;num#<?php echo htmlentities($vo['num']); ?>">驳回</a><?php endif; ?></td></tr><?php endforeach; ?></tbody></table><?php if(empty($list) || (($list instanceof \think\Collection || $list instanceof \think\Paginator ) && $list->isEmpty())): ?><span class="notdata">没有记录哦</span><?php else: ?><?php echo (isset($pagehtml) && ($pagehtml !== '')?$pagehtml:''); ?><?php endif; ?></div><script>$(".J_bohui").click(function () {
        ...
        ,yes: function(index, layero) {
            $.ajax({
                ...
                data: {
                    ...
                    'num': num,
                    'remark': $(".J_remark").val(),
                    '_csrf_': "<?php echo systoken('admin/deal/do_deposit'); ?>"
                },
